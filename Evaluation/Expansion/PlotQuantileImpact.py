...
if __name__ == "__main__":
    """ SELECT Dice with alpha 0.1,dynamic (mapping_id 16):
    16	Iterative	0.95	Dice	0.2	True	0.0
    58	Iterative	0.95	FactPair-Sim	0.2	True	1.0, Fact pair sim 0.2,dynamic """
    ...
    mapping_df =sql_con.query_table(query="SELECT * FROM MappingSetup; ")
    ...
    res_df = res_df[~((res_df['metric'] == 'Dice') & (res_df['importance_parameter'] != 0.8))]
    #print(calc_best_anchor_quantile(res_df,['metric','anchor_quantile']).to_latex())

    #print(calc_best_importance_weight(res_df,['metric','importance_parameter']).to_latex())

    #print(db_config_df[['file_name','db1','db2','equal_facts_perc','nr_equal_facts','nr_poss_facts']].to_latex())
    ...
    overlap_df = calc_overlap_perc_all_resources(res_df,['metric','dynamic','importance_parameter','anchor_quantile'],NR_DBS=5)
