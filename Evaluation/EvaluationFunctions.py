...
def plot_std_dev_over_runs(res_df,PLOT_FIG,gb_cols,nr_total_facts):
    ...
    res_df = res_df[gb_cols + ['db_config_id', 'common_facts']]
    ...
    r['std_dev'] = np.sqrt(r['common_facts'] / 8) * 100 /nr_total_facts
    r = r.reset_index()
    if PLOT_FIG:
...
def calc_overlap_perc_all_resources(res_df,gb_cols,NR_DBS,special_cols):
    """ Calculate the average by collecting common facts  and divide by nr of elements (using overlap perc. is inaccurate)"""
    df = res_df[['mapping_id', 'metric', 'dynamic','anchor_quantile','importance_parameter','common_facts','computed_mappings','max_tuples','runtime','struct_ratio'] + special_cols]
    ...
    group_df['common_facts'] = group_df['common_facts'] / group_df['size']
    return group_df
...
def plot_overlap_per_mapping(overlap_df,color=None,group=None):
    for ind,row in overlap_df.iterrows():
        overlap_df['name'] = overlap_df.apply(
            lambda row: f"q={row['anchor_quantile']},w={row['importance_parameter']},m={row['metric']},d={row['dynamic']}",
            axis=1)
...
def plot_overlap_per_resource(res_df, PLOT_FIG):
    res_df = res_df[['metric','dynamic','anchor_quantile','importance_parameter','overlap_perc','db_config_id']]
    if PLOT_FIG:
        for group,df in res_df.groupby(['db_config_id']):
            #df['name'] = f"q={df['anchor_quantile']},w={df['importance_parameter']},m={df['metric']}"
            df['name'] = df.apply(
                lambda row: f"q={row['anchor_quantile']},w={row['importance_parameter']},m={row['metric']},d={row['dynamic']}", axis=1)
            fig = px.bar(df,x='name',y="overlap_perc",title=str(group[0]),color='metric')
            fig.show()

def plot_best_overlap_per_resource(res_df, PLOT_FIG):
    res_df = res_df[['metric','dynamic','anchor_quantile','importance_parameter','overlap_perc','db_config_id','file_name'
        ,'equal_facts_perc']]
...
def calc_best_anchor_quantile(res_df,gr_cols):
    #res_df_w_alpha = res_df_w_alpha[['anchor_quantile','common_facts','use','file_name','nr_poss_facts','metric',
    ...
    gr_size = s.iat[0]
    group_df['avg_overlap'] = round(group_df['common_facts'] * 100 / (group_df['nr_poss_facts']),2)
...
def calc_best_importance_weight(res_df,gr_cols):
    ...
    group_df = group_df.sum(numeric_only=True)
    gr_size = s.iat[0]
    group_df['avg_overlap'] = round(group_df['common_facts'] * 100 / (group_df['nr_poss_facts']),3)
    group_df['avg_rt'] = round(group_df['runtime'] / (gr_size),1)
    group_df['avg_comp_mappings'] = round(group_df['computed_mappings'] / (gr_size * 1000))
...
def count_correct_mappings(res_df, config_df,gb_cols):
    ...
    for index, row in res_df.iterrows():
        ...
        # Calculate the number of mappings (eq. number of elements) and the number of correct mappings (c -> c)
