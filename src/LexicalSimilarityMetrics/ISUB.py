...
class IsubStringMatcher(LexicalSimilarityMetric):
    ...
    def compute_lexical_similarity(self, element_name1, element_name2):
        ...
        # if both elements are integers String-Matching will have problems -> return closeness of both ints then
        ...
        count_lcs = 0
        sm = difflib.SequenceMatcher(None, element_name1, element_name2)
        matching_blocks = sm.get_matching_blocks()
        # convert to list to reduce string sice
        l_st1 = len(element_name1)
        l_st2 = len(element_name2)
        if not l_st1 and not l_st2:
            return 1
        elif bool(l_st1) != bool(l_st2):
            return 0

        element_list1 = list(element_name1)
        element_list2 = list(element_name2)
        ...
        for st1_ind, st2_ind, size in matching_blocks:
            ...
            count_lcs += size
            element_list1[st1_ind:st1_ind + size] = [None] * size
            element_list2[st2_ind:st2_ind + size] = [None] * size

        comm = 2 * count_lcs / (l_st1 + l_st2)

        # reduce strings of lcs matching
        diff1 = len([c for c in element_list1 if c]) / l_st1
        diff2 = len([c for c in element_list2 if c]) / l_st2
