...
class Mapping():
    def __init__(self, paths, exp_name,expansion_strategy,sim_name, similarity_metric):
        ...
        self.db1_renamed_facts = Databases.DB_Instance(paths.db1_facts, name)

        self.db_merged_facts = Databases.DB_Instance(paths.merge_facts, name)
        self.db_merged_results = Databases.DB_Instance(paths.merge_results, name)

        self.db1_unravelled_results = Databases.DB_Instance(paths.db1_results, name)
        self.db2_unravelled_results = Databases.DB_Instance(paths.db2_results, name)
            ...
            for row_ind, row in df.iterrows():
                ...
                for term_name, cols in temp_dict.items():
                    ...
                    if term_name in terms:
                        ...
                        term_obj = Terms.Term(term_name, file_name, cols, curr_record)
                        terms[term_name] = term_obj

                    curr_record.add_term(term_obj,cols)
