...
def dynamic_jaccard_index(term1, term2, sub_rids):
    ...
    for record in sub_rids.keys():
        if record.db == "facts-db1":
            file_rids1.setdefault(record.file_name,list()).append(record.rid)
        else:
            file_rids2.setdefault(record.file_name, list()).append(record.rid)
    ...
    total_occurrences += sum(len(x) for x in term1.occurrences.values())
    total_occurrences += sum(len(x) for x in term2.occurrences.values())
